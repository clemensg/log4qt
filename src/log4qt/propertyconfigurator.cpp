...
void PropertyConfigurator::configureFromProperties(const Properties
    &rProperties,
    LoggerRepository *pLoggerRepository)
{
...
void PropertyConfigurator::configureGlobalSettings(const Properties
    &rProperties,
    LoggerRepository *pLoggerRepository) const
{
  Q_ASSERT_X(pLoggerRepository, "PropertyConfigurator::configureGlobalSettings()",
  ...
  if (value.isNull()) {
    ...
    if (!value.isNull())
      logger()->warn("[%1] is deprecated. Use [%2] instead.", key_config_debug,
                     key_debug);
  }
  if (!value.isNull()) {
  ...
  if (!value.isNull()) {
    pLoggerRepository->setThreshold(OptionConverter::toLevel(value,
                                    Level::ALL_INT));
...
void PropertyConfigurator::configureNonRootElements(const Properties
    &rProperties,
    LoggerRepository *pLoggerRepository)
{
  Q_ASSERT_X(pLoggerRepository,
             "PropertyConfigurator::configureNonRootElements()",
             "pLoggerRepository must not be null.");
...
void PropertyConfigurator::configureRootLogger(const Properties &rProperties,
    LoggerRepository *pLoggerRepository)
{
  Q_ASSERT_X(pLoggerRepository, "PropertyConfigurator::configureRootLogger()",
             "pLoggerRepository must not be null.");
  ...
  if (value.isNull()) {
    ...
    if (!value.isNull())
      logger()->warn("[%1] is deprecated. Use [%2] instead.", key_root_category,
                     key_root_logger);
  }
...
void PropertyConfigurator::parseAdditivityForLogger(const Properties
    &rProperties,
    Logger *pLogger,
    const QString &rLog4jName) const
{
  ...
  QString value = OptionConverter::findAndSubst(rProperties, key);
  logger()->debug("Parsing additivity for logger: key '%1', value '%2'", key,
                  value);
  if (!value.isEmpty()) {
    bool additivity = OptionConverter::toBoolean(value, true);
    logger()->debug("Setting additivity for logger '%1' to '%2'", pLogger->name(),
...
LogObjectPtr<Appender> PropertyConfigurator::parseAppender(
  ...
  const QString &rName)
{
  ...
  if (value.isNull()) {
    LogError e = LOG4QT_ERROR(
                   QT_TR_NOOP("Missing appender definition for appender named '%1'"),
  ...
  if (!p_appender) {
    LogError e = LOG4QT_ERROR(
                   QT_TR_NOOP("Unable to create appender of class '%1' namd '%2'"),
  ...
  AppenderSkeleton *p_appenderskeleton = qobject_cast<AppenderSkeleton *>
                                         (p_appender);
...
LogObjectPtr<Layout> PropertyConfigurator::parseLayout(const Properties
    &rProperties,
    const QString &rAppenderKey)
{
  Q_ASSERT_X(!rAppenderKey.isEmpty(), "PropertyConfigurator::parseLayout()",
             "rAppenderKey must not be empty.");
  ...
  if (value.isNull()) {
    LogError e = LOG4QT_ERROR(
                   QT_TR_NOOP("Missing layout definition for appender '%1'"),
  ...
  if (!p_layout) {
    LogError e = LOG4QT_ERROR(
                   QT_TR_NOOP("Unable to create layoput of class '%1' requested by appender '%2'"),
...
{
  Q_ASSERT_X(pLogger, "PropertyConfigurator::parseLogger()",
             "pLogger must not be null.");
  Q_ASSERT_X(!rKey.isEmpty(), "PropertyConfigurator::parseLogger()",
             "rKey must not be empty.");
...
void PropertyConfigurator::setProperties(const Properties &rProperties,
    ...
    QObject *pObject)
{
  Q_ASSERT_X(!rPrefix.isEmpty(), "PropertyConfigurator::setProperties()",
             "rPrefix must not be empty.");
  Q_ASSERT_X(pObject, "PropertyConfigurator::setProperties()",
             "pObject must not be null.");
...
{
  Q_ASSERT_X(!mpConfigureErrors, "PropertyConfigurator::startCaptureErrors()",
             "mpConfigureErrors must be empty.");
...
{
  Q_ASSERT_X(mpConfigureErrors, "PropertyConfigurator::stopCaptureErrors()",
             "mpConfigureErrors must not be empty.");
