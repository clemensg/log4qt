...
QString DateTime::formatDateTime(const QString &rFormat) const
{
  ...
  for (i = 0; i < rFormat.length(); i++) {
    ...
    if (expected == quote) {
      if (c == quote) {
        Q_ASSERT_X(i > 0, "DateTime::toString()",
                   "Found quote with status quote at i = 0");
...
QString DateTime::formatToken(const QString &rToken, bool am_pm) const
{
  ...
  if (rToken.startsWith(QLatin1String("dddd"))) {
    ...
    result = QString::number(date().day()).rightJustified(2, QLatin1Char('0'),
             true);
    used = 2;
  }
    ...
    result = QString::number(date().month()).rightJustified(2, QLatin1Char('0'),
             true);
    used = 2;
  }
    ...
    result = QString::number(date().year() % 100).rightJustified(2,
             QLatin1Char('0'), true);
    used = 2;
  }
  ...
  // Qt time format strings
    ...
    result = QString::number(time().minute()).rightJustified(2, QLatin1Char('0'),
             true);
    used = 2;
  }
  else if (c == (QLatin1Char('m'))) {
    ...
    result = QString::number(time().second()).rightJustified(2, QLatin1Char('0'),
             true);
    used = 2;
  }
    ...
    result = QString::number(time().msec()).rightJustified(3, QLatin1Char('0'),
             true);
    used = 3;
  }
  ...
  // Extension for week number
    ...
    result = QString::number(date().weekNumber()).rightJustified(2,
             QLatin1Char('0'), true);
    used = 2;
  }
