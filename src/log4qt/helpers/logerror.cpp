...
{
#if QT_VERSION < QT_VERSION_CHECK(5, 0, 0)
  return QCoreApplication::translate(mContext.toLatin1(),
                                     mMessage.toUtf8().data(), 0, QCoreApplication::UnicodeUTF8);
#else
  return QCoreApplication::translate(mContext.toLatin1(),
                                     mMessage.toUtf8().data(), 0);
#endif
}
...
QString LogError::toString() const
{
  ...
  if (!mCausingErrors.isEmpty()) {
    QString causing_errors_str = QLatin1String(": ") + mCausingErrors.at(
                                   0).toString();
    int i = 1;
    while (i < mCausingErrors.count()) {
      causing_errors_str.append(QLatin1String(", ")).append(mCausingErrors.at(
            i).toString());
      i++;
    }
