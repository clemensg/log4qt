...
private:
  ...
  BasicPatternConverter &operator=(const BasicPatternConverter
                                   &rOther); // Not implemented
...
private:
  ...
  LoggerPatternConverter &operator=(const LoggerPatternConverter
                                    &rOther); // Not implemented
                                       ...
                                       const QString &rOption)
{
  Q_ASSERT_X(mConversionCharacters.indexOf(rChar) >= 0,
             "PatternFormatter::createConverter", "Unknown conversion character");
...
int PatternFormatter::parseIntegerOption(const QString &rOption)
{
  ...
  if (!ok) {
    LogError e = LOG4QT_ERROR(
                   QT_TR_NOOP("Option '%1' cannot be converted into an integer"),
...
void PatternConverter::format(QString &rFormat,
                              const LoggingEvent &rLoggingEvent) const
{
...
QString LiteralPatternConverter::convert(const LoggingEvent &rLoggingEvent)
const
{
