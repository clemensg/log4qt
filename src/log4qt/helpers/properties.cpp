...
void Properties::parseProperty(const QString &rProperty,
                               int line)
{
  Q_ASSERT_X(rProperty == trimLeft(rProperty), "parseProperty()",
             "rProperty has leading spaces");
  ...
  Q_ASSERT_X(value_escape_codes.length() == value_escape_chars.length(),
  ...
  Q_ASSERT_X(key_escape_codes.length() == key_escape_chars.length(),
      ...
      case VALUEESCAPE_STATE: {
        ...
        if (convert >= 0) {
          *p_string += value_escape_chars.at(convert);
          ...
          logger()->warn("Unknown escape sequence '\\%1' in value of property starting at line %2",
                         QString(c), line);
          *p_string += c;
          state = VALUE_STATE;
        }
...
QDebug operator<<(QDebug debug, const Properties &rProperties)
{
  debug.nospace() << "Properties("
                  ...
                  << "properties:" << *reinterpret_cast<const QHash <QString, QString > *>
                  (&rProperties)
                  << ")";
  return debug.space();
}
