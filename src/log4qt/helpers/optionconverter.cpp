  ...
  while (i < value.length()) {
    ...
    if (begin == -1) {
      ...
      if (end == -1) {
        ...
        result += findAndSubst(rProperties, value.mid(begin + begin_length,
                               end - begin - end_length - 1));
        i = end + end_length;
      }
    }
  }
                                ...
                                bool *p_ok)
{
  ...
  if (p_ok)
    *p_ok = false;
  LogError e = LOG4QT_ERROR(
                 QT_TR_NOOP("Invalid option string '%1' for a boolean"),
                                   ...
                                   bool *p_ok)
{
  ...
  if (!ok || value < 0 || s.length() > i + 2) {
    LogError e = LOG4QT_ERROR(
                   QT_TR_NOOP("Invalid option string '%1' for a file size"),
                   CONFIGURATOR_INVALID_OPTION_ERROR,
                   "Log4Qt::OptionConverter");
    e << rOption;
    logger()->error(e);
                           ...
                           bool *p_ok)
{
  ...
  LogError e = LOG4QT_ERROR(
                 QT_TR_NOOP("Invalid option string '%1' for an integer"),
                 CONFIGURATOR_INVALID_OPTION_ERROR,
                 "Log4Qt::OptionConverter");
  e << rOption;
  logger()->error(e);
