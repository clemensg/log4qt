...
void FileAppender::activateOptions()
{
  ...
  if (mFileName.isEmpty()) {
    LogError e = LOG4QT_QCLASS_ERROR(
                   QT_TR_NOOP("Activation of Appender '%1' that requires file and has no file set"),
...
bool FileAppender::checkEntryConditions() const
{
  ...
  if (!mpFile || !mpTextStream) {
    LogError e = LOG4QT_QCLASS_ERROR(
                   QT_TR_NOOP("Use of appender '%1' without open file"),
...
void FileAppender::closeFile()
{
  ...
  if (mpFile)
    logger()->debug("Closing file '%1' for appender '%2'", mpFile->fileName(),
                    name());
...
bool FileAppender::handleIoErrors() const
{
  ...
  LogError e = LOG4QT_QCLASS_ERROR(
                 QT_TR_NOOP("Unable to write to file '%1' for appender '%2'"),
...
{
  Q_ASSERT_X(mpFile == 0 &&
             mpTextStream == 0, "FileAppender::openFile()",
             "Opening file without closing previous file");
  ...
  logger()->debug("Opened file '%1' for appender '%2'", mpFile->fileName(),
                  name());
}
...
bool FileAppender::removeFile(QFile &rFile) const
{
  ...
  LogError e = LOG4QT_QCLASS_ERROR(
                 QT_TR_NOOP("Unable to remove file '%1' for appender '%2'"),
                              ...
                              const QString &rFileName) const
{
  ...
  LogError e = LOG4QT_QCLASS_ERROR(
                 QT_TR_NOOP("Unable to rename file '%1' to '%2' for appender '%3'"),
